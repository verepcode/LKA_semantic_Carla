...
DriveController::DriveController(
    ...
    : /* Initialize variables */
      ...
      m_MaxTorqueLimit(
          2000),  // Hardcoded limit, as it can become a safety hazard
...
void DriveController::setAbsoluteSetpoint(
    longint n_pos, int n_max_torque, float f_Kp, float f_Kd) {
    /* Lock Tasks List */
    std::lock_guard<std::mutex> guard(m_mutexSetpoint);
...
void DriveController::DoUpdateCycle() {
    ...
    if (m_bTrackingErrorFault) {
        ...
        /* Calculating Torque based on PD equation */

        /* Proportional component */
        nTorqueSetpoint = round((m_unKp * posTrackingError));

        /*  Differential component */
        if (dt != 0) {
            /* dt cannot zero, as we would go into "divided-by-zero" err */
            nTorqueSetpoint += round((m_unKd * delta_error / dt));
        }
    ...
    if (!m_bIsSetpointTrackingEnabled) {
        ...
        if (abs(delta_error) > m_minPosThreshold) {
            ...
            nTorqueSetpoint =
                m_nResistiveTorque * m_unResistiveKd * delta_error;
        } else {
            nTorqueSetpoint = 0;
        }
    }
